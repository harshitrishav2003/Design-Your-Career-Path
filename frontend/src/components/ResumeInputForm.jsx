...
const removeSkill = (index) => {
    ...
    const updatePreview = (updatedForm) => {
        fetch('https://design-your-career-path.vercel.app/api/generate-resume', {
            method: 'POST',
       ...
       .then(data => {
            ...
            if (data.resumeUrl) {
                setPreviewUrl(`https://design-your-career-path.vercel.app${data.resumeUrl}?t=${new Date().getTime()}`); // Add a timestamp to avoid caching
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('https://design-your-career-path.vercel.app/api/generate-resume', formData);
            
            console.log(response.data.resumeUrl);
            
            // Check if the response contains a resume URL
            if (response.data && response.data.resumeUrl) {
                const downloadLink = `https://design-your-career-path.vercel.app${response.data.resumeUrl}`;
